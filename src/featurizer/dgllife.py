    ...
    **We assume the resulting DGLGraph will not contain any virtual nodes.**
    ...
    def __init__(
        ...
        self._featurizer = ConcatFeaturizer(
            [
                partial(
                    atom_type_one_hot, allowable_set=atom_types, encode_unknown=True
                ),
    ...
    def __call__(self, mol):
        ...
        return {
            self._atom_data_field: torch.as_tensor(atom_features.astype(np.float32))
        }
...
class WeaveEdgeFeaturizer(object):
    ...
    * Whether a pair of atoms belongs to a same ring
    ...
    def __call__(self, mol):
        ...
        distance_indicators = (
            distance_matrix > torch.arange(0, self._max_distance).float()
        ).float()
        ...
        for bond in mol.GetBonds():
            bond_type_encoding = torch.tensor(
                bond_type_one_hot(bond, allowable_set=self._bond_types)
            ).float()
        ...
        return {
            self._edge_data_field: torch.cat(
                [distance_indicators, bond_indicators, ring_mate_indicators], dim=1
            )
        }
...
class PretrainBondFeaturizer(object):
    ...
    * bond direction
    ...
    def __call__(self, mol):
        ...
        return {
            "bond_type": edge_features[:, 0],
            "bond_direction_type": edge_features[:, 1],
        }
